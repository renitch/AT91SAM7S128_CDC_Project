...
# - 27. Oct. 2006  - Toolchain-prefix (mth), linker-scripts-path
...
ASRCARM += Compil/srcWinARM/swi_handler.S
...
EXTRAINCDIRS = include Compil/srcWinARM

# List any extra directories to look for library files here.
#     Each directory must be seperated by a space.
EXTRA_LIBDIRS = Compil/srcWinARM
...
ifeq ($(FORMAT),ihex)
#build: elf hex lss sym out
build: elf hex out
hex: $(TARGET).hex
IMGEXT=hex
else 
ifeq ($(FORMAT),binary)
#build: elf bin lss sym out
build: elf hex out
bin: $(TARGET).bin
IMGEXT=bin
else 
...
program: $(TARGET).$(IMGEXT)
	@echo
ifeq ($(SUBMDL),AT91SAM7S64)
	@echo "Programming SAM7S64 with uVision"
	C:\Keil\uv3\Uv3.exe -f uvisionflash64.Uv2 -ouvisionflash.txt
endif
ifeq ($(SUBMDL),AT91SAM7S256)
	@echo "Programming SAM7S256 with uVision"
	C:\Keil\uv3\Uv3.exe -f uvisionflash256.Uv2 -ouvisionflash.txt
endif
else
ifeq ($(FLASH_TOOL),OPENOCD)
# Program the device with Dominic Rath's OPENOCD in "batch-mode", needs cfg and "reset-script".
# see files in distribution
program: $(TARGET).$(IMGEXT)
	@echo
